...
<ModalItem v-if="$store.state.photo.imgsArr" ref="modal" :image=$store.state.photo.imgsArr[curIdx] />
...
export default {
    ...
    methods: {
        showPic(id) {
            ...
            this.curIdx = this.$store.state.photo.id2idx[id];
            this.$nextTick(() => {
                this.$forceUpdate();
                this.$refs.modal.showImage();
            })
